...
def trackmeplease(datatotrack):
    ...
    try:
        conn.execute(
            '''CREATE TABLE IF NOT EXISTS MOVIES (TITLE TEXT PRIMARY KEY NOT NULL, YEAR INT  NOT NULL);''')
...
def harvestkey():
    with open("/home/student/mycode/apisqlite/omdb.key") as apikeyfile:
...
def main():
    ...
    while True:
        ...
        if answer in ["1", "2"]:
            searchstring = input(
                "Search all movies in the OMDB. Enter search string: ")
